...
\begin{figure*}
  ...
  Slice 20 percent of the max-min range in the "z" position of particles belonging to the main structure, centered at halo defined position (calculated with most bounded particle) }
  % definition of the input positions
  % Gets the length of the main structure particles /// 0 - Principal halo, 1 - DM
  %  cut = subSn.group.GroupLenType[0,1]
  % graphs are calculated using the following YT command
...
Radial profile was sampled at redshift 0 and there are two sets of lines showing MHD and DM-only versions of the axial ratios profile}
\label{fig:DM_MHD}
\end{figure} 
